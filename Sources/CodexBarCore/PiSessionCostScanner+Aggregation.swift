        ...
        range: CostUsageScanner.CostUsageDayRange) -> CostUsageDailyReport?
    {
        ...
        for dayKey in dayKeys {
            ...
            for modelName in models.keys.sorted() {
                ...
                // Fork: keep pricing provenance with the amount so sync never has to
                // re-infer it from a mutable catalog.
                let hasExactPricing = switch provider {
                case .codex: CostUsagePricing.hasExactCodexPricing(modelName, modelsDevCatalog: nil)
                case .claude: CostUsagePricing.hasExactClaudePricing(modelName, modelsDevCatalog: nil)
                default: true
                }
                let hasCost = packed.costSampleCount > 0
                breakdown.append(CostUsageDailyReport.ModelBreakdown(
                    modelName: modelName,
                    costUSD: hasCost ? Double(packed.costNanos) / self.costScale : nil,
                    totalTokens: packed.totalTokens,
                    requestCount: packed.usageSampleCount,
                    isEstimated: hasCost && !hasExactPricing ? true : nil))
            }
