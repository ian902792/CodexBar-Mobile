...
### Changed
- Upstream sync: merge every upstream release from v0.58.0 through v0.64.1 into Mac `0.64.1.1` (Sparkle build `153.1`). The paired iOS version is unchanged at 1.23.0.

### Fixed
- Fork integration: keep the CloudKit sync layer, Production entitlements, mobile bridge, collision-safe versioning, signing identity, appcast, fork README, and the two-layer CI policy while taking upstream's provider work.
- Mobile sync: ElevenLabs and LLMProxy moved to upstream's plugin lane, so their snapshots now travel over the generic details lane like Deepgram; the sync payload format is unchanged.
