...
### Added
- Usage Bars widget (small/medium): per-rate-window bars, used/remaining percent and reset countdown in the provider tint on an always-dark card; optional provider picker (`AppEntity`), defaults to the highest usage. Windows keep their synced order (Session above Weekly); values follow the app's Show remaining usage setting via the `group.com.ian902792.codexbar.mobile` App Group; reset countdowns use compact English units (`3d 1h`, `1h 49m`) refreshed by per-minute timeline entries; the medium layout labels each bar with its own window, value and countdown. Widget summaries now carry localized windows and the synced icon tint; the 6-provider cap on widget summaries is removed (layouts already limit their rows). `ProviderColorPalette`/`ProviderWindowLabel` moved to `CodexBarWidgetShared/`.
