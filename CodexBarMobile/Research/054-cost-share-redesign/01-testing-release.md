...
状态：done。目标版本：iOS 2.0.0 (211)。实现 PR 已合并到 `mobile-dev`，TestFlight build 已交付。
...
- 第八轮 PR 审查修复：iPad 横向列表详情布局中的 Provider 行与紧凑卡片共享同一隐私脱敏 helper，开启“Hide personal information”后，窗口尺寸切换不会暴露账号邮箱。新增 helper 回归测试；因此 build 211 成为唯一候选，完整测试已通过；归档、上传与绑定结果见下。
...
- 最终 Archive：`/Volumes/StudioSSD/Developer/BuildScratch/CodexBar/Archives/CodexBarMobile-2.0.0-211.xcarchive`；归档于 2026-09-14 16:33:02 PDT 开始，当时发布分支 HEAD 为 clean-reviewed `f53882a1c0b1762f8882f14f070f520ee1a0ee8d`（tree `35b6ab7d23d579ccc970b6ab73e5aab84e9fac3a`）。该 head 相对最终产品代码 commit `61f87a77a480fe0ca6d9f79a801a40d5bfcb1288` 仅增加 Research 文档，iOS 代码、资源与 build settings 无 diff。归档排序后的逐文件 SHA-256 manifest digest 为 `8f78923e7b43338bd96bd3bbff497f3eb5dccb11d0a7718fe805626bcc7b0f70`，主 App executable SHA-256 为 `ad7e704d7502a6f2ae38d46438634b3e2234866ac9c2cb936bdd2f3d67fa3f77`；archive log 为 `/Volumes/StudioSSD/Developer/BuildScratch/CodexBar/logs/iOS27-build211-archive.log`。归档主 App、Widget、Push Extension、Sync framework 均为 `2.0.0 (211)`，`codesign --verify --deep --strict` 通过，主 App CloudKit environment 为 `Production`。
